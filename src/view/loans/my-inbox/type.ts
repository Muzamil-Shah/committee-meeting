...
[];
export interface MomObservation{
  id: number;
  agenda: { title: string; subject: string };
  remarks: string;
  decision: string;
}
export interface MyInboxMOM {
  department: string;
  background: string;
  committeesUser: CommitteeMember[] | null[];
  attendanceList: CommitteeMember[] | null[];
  observation: MomObservation[];
}
export interface MyInboxI {
  id: number;
  meeting: IMeeting;
  agenda: InboxAgendaI[];
  nfa: InboxNFAI[];
